...
object ConsumingDataGenerator {
  private val FILE_PATH = "D:/spark_parogram/Spark-scala/scalatext/src/main/scala/SparkSql/userData/sample_consuming_data.txt"
  ...
  private def generateDataFile(filePath : String, recordNum: Int): Unit = {
    ...
    try {
      writer = new FileWriter(filePath,true) //写入文件，true为追加
      val rand = new Random()
      for (i <- 1 to recordNum) {
        ...
        writer.write(i + " " + recordDate + " " + productID+ " " + price + " " + userID)
